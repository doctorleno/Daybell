...
export default function Home({accountName,accountId}: {accountName:string;accountId:string}) {
    ...
    async function load() { try {
        ...
        if (r.status===401){setEntries([]);stopRinging();setEnabled(false);setError("Your session ended. Sign in again to continue.");return;}
        if (!r.ok)
            throw Error();
 ...
 <div className="accountbar"><span title={accountName}>Private calendar · {accountName}</span><a href="/signout-with-chatgpt?return_to=%2F" onClick={stopRinging}>Sign out</a></div><main><section className="heading"><div><div className="eyebrow"><Sparkles size={15}/> A GOOD DAY STARTS HERE</div><h1>Big plans.<br /><span>Little reminders.</span></h1><p>Your time, your rhythm. Let's make today happen.</p></div><button className="primary" onClick={() => create()}><Plus size={18}/>Make a plan <ArrowUpRight size={18}/></button></section>
